package com.caixa.notaderelease.api.security.controller;
...
public class AuthenticationRestController {
    ...
    public ResponseEntity<?> createAuthenticationToken(@RequestBody JwtAuthenticationRequest authenticationRequest) throws AuthenticationException {
        ...
        final Authentication authentication = authenticationManager.authenticate(
                new UsernamePasswordAuthenticationToken(
                        authenticationRequest.getMatricula(),
                        authenticationRequest.getPassword()
                )
        );
        SecurityContextHolder.getContext().setAuthentication(authentication);
        final UserDetails userDetails = userDetailsService.loadUserByUsername(authenticationRequest.getMatricula());
        final String token = jwtTokenUtil.generateToken(userDetails);
        final User user = userService.findByMatricula(authenticationRequest.getMatricula());
    ...
    public ResponseEntity<?> refreshAndGetAuthenticationToken(HttpServletRequest request) {
        ...
        final User user = userService.findByMatricula(username);
